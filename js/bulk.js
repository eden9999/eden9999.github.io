...
// Gene set filters: add new sets here to enable automatic button and filter handling.
// This object is the single source of truth for all gene set filter buttons and logic.
const geneSetFilters = {
  // Example gene set: Transcription Factors
  TF: { enabled: false, genes: null, csv: "/uploads/TF.csv", btn: "btn-tf-intersect" },
  // Example gene set: Hedgehog pathway
  Hedgehog: { enabled: false, genes: null, csv: "/uploads/hedgehog.csv", btn: "btn-hedgehog" },
  // Example gene set: Thymus
  Thymus: { enabled: false, genes: null, csv: "/uploads/thymus.csv", btn: "btn-thymus" }
  // Add future gene sets here in the same format.
};
...
function resetAllBtnStates() {
    ...
    Object.values(geneSetFilters).forEach(obj => {
        obj.enabled = false;
        setBtnActive(obj.btn, false);
    });
}
